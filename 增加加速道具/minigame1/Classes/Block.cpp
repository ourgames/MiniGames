...
void BaseBlock::addAvoidEffect(IDisplayObject *pCollisionTarget)
{
        ...
        int idnum = (int)EffectType::BLOCKAVOIDSCORE + EFFECTIDBASE;    
        std::string eftypestr = CommonUtil::itos(idnum);
...
//    thisCollisionBox.origin = this->convertToWorldSpace(thispositionNode);
    ...
    collisionBox1.size.width = pCollisionTarget->getCollisionBox().size.width;
    collisionBox1.size.height = pCollisionTarget->getCollisionBox().size.height;
    position1 = pCollisionTarget->convertToWorldSpace(Point::ZERO);
    collisionBox1.origin.x = position1.x - collisionBox1.size.width/2;
    collisionBox1.origin.y = position1.y - collisionBox1.size.height/2;
    ...
    position = this->convertToWorldSpace(Point::ZERO);
    thiscollisionBox.origin.x = position.x - thiscollisionBox.size.width/2;
    thiscollisionBox.origin.y = position.y - thiscollisionBox.size.height/2;
